...
export const loginUser = catchAsync(async (req, res, next) => {
  ...
  const isOauth = false;
  await sendToken(user, 200, true, isOauth, req, res);
});
...
export const currentUser = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    ...
    return res.status(401).json({
      success: false,
      message: "User not found. Tokens cleared.",
    });
  }
...
// ========================
// Update access token (Socket.IO)
// ========================
export const updateAccessToken = catchAsync((req, res, next) => {
  const { access_token } = req.body;

  if (!access_token) return next(new AppError("Access token is required", 400));
  ...
  res.cookie("access_token", access_token, optionsAccessToken).status(200).json({
    success: true,
    access_token,
  });
});
